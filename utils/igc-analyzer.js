...
class IGCAnalyzer {
    ...
    compute(fixes) {
        ...
        for (let f of fixes) {
            ...
            const coords_len = coords.push(_coord);
            if (coords_len > 1) {
                totalDistance += distanceTo(_coord, coords[coords_len - 2]);
            }
            f['distance'] = totalDistance
        }

        let dt = 20; // dt est le péridoe d'exploration fixé à 20 secondes par défaut  (ligne 39)
        let n = coords.length;
        // in java code, period is cast to integer. In javascript, it seems that Math.floor or Math.trunc are used
        ...
        for (let i = 1; i < n; i++) {
            ...
            if (coords[i].ele < min_ele) min_ele = coords[i].ele;
            else if (coords[i].ele - min_ele > max_dz_positive) max_dz_positive = coords[i].ele - min_ele;
        }
        ...
        for (let i = 1; i < n; i++) {
            ...
            if (ds === 0.0) valProgress = 0.0;
            else if (dp > ds) valProgress = 1.0;
            else valProgress = dp / ds;
        ...
        this.bestThermalClimb = _bestThermalClimb;
        ...
        function addDetails(idxStart, idxEnd, category) {
            ...
            for (let i = idxStart; i < idxEnd; i++) {
                ...
                dt = t[i + 1] - t[i];
                if (dz > 0) total_dz_positive += dz;
                else if (dz < 0) total_dz_negative += dz;
            ...
            if (divisor === 0) segment.efficiency = -1;
            else segment.efficiency = Math.round((100.0 * dz) / divisor);
        ...
        function fillProgressList() {
            ...
            const hTkoff =
                String(dateTkoff.getHours()).padStart(2, '0') +
                ':' +
                String(dateTkoff.getMinutes()).padStart(2, '0') +
                ':' +
                String(dateTkoff.getSeconds()).padStart(2, '0');
            ...
            const hLand =
                String(dateLand.getHours()).padStart(2, '0') +
                ':' +
                String(dateLand.getMinutes()).padStart(2, '0') +
                ':' +
                String(dateLand.getSeconds()).padStart(2, '0');
            ...
            for (let i = 0; i < nbThermals; i++) {
                ...
                let hEnd =
                    String(dateEnd.getHours()).padStart(2, '0') +
                    ':' +
                    String(dateEnd.getMinutes()).padStart(2, '0') +
                    ':' +
                    String(dateEnd.getSeconds()).padStart(2, '0');
                ...
                let progTh = new progSegment(
                    'T',
                    finalThermals[i].finish_time,
                    hEnd,
                    elapsedFormatted,
                    finalThermals[i].finish_altitude,
                    finalThermals[i].deltaAlt,
                    finalThermals[i].climbAverage,
                    segCoords
                );
                ...
                let featurePoint = {
                    ...
                    geometry: {
                        type: 'Point',
                        coordinates: [
                            Math.round(fixes[finalThermals[i].idxEnd].longitude * 100000) / 100000,
                            Math.round(fixes[finalThermals[i].idxEnd].latitude * 100000) / 100000,
                        ],
                    },
                    properties: {
            ...
            for (let i = 0; i < nbGlides; i++) {
                ...
                let progGl = new progSegment(
                    'G',
                    finalGlides[i].finish_time,
                    hEnd,
                    elapsedFormatted,
                    finalGlides[i].finish_altitude,
                    (finalGlides[i].distance_metres / 1000).toFixed(2),
                    Math.round(finalGlides[i].average_speed),
                    segCoords
                );
                ...
                let longMiddle =
                    fixes[finalGlides[i].idxEnd].longitude + (fixes[finalGlides[i].idxStart].longitude - fixes[finalGlides[i].idxEnd].longitude) / 2;
                let glToRight;
                glToRight = fixes[finalGlides[i].idxStart].longitude < fixes[finalGlides[i].idxEnd].longitude;
                let featurePoint = {
                    type: 'Feature',
                    geometry: {
                        type: 'Point',
            ...
            if (nbGlides > 0) _avgTransSpeed = totSpeed / nbGlides;
            else _avgTransSpeed = 0;
        ...
        function interpolate(c, other, delta) {
            ...
            return new radCoord(lat, lon, ele);
        }
